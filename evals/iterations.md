...
| llm-v3 | 2026-09-16 | routing: Sonnet 5 prompt v2 first; escalate to Opus 5 (prompt v2, effort high) when the cheap pass says has_facts but its anchors cannot be located, a span validator flag fires, or the call errors; no-facts verdicts never escalate; both passes kept in provenance | L2 (8 residual anchor failures), L5 | `labeled-v3_20260916-001818_90b33c` | 120 (35 human) | 0.975 (all, n=120) — llm-v2 0.912 | — | 72.5% | 55.0% | 0.964 verbatim / 0.986 support | $0.331 (CLI est.; 8 Opus calls, $9.35 total) | 37.4 s (escalated docs ≈ 99 s) |
| llm-v4 | 2026-09-16 | prompt v3 = v2 with record citations as objects {as_printed, prefix, pages} (wire schema 2): the printed string is copied verbatim and any list/range expansion goes into `pages`; downstream still receives strings (`as_printed`) | L3 (list/range expansion, 152 on v2) | `labeled-v5_20260916-132745_3eac2d` | 120 (35 human) | 0.953 (all, n=120) | — | 70.0% | 59.2% | 0.992 verbatim / 0.995 support | $0.402 (CLI est.) | 71.5 s |
| llm-v5 | 2026-09-16 | llm-v4 + the llm-v3 route (Opus 5 on anchor failure / validator flag / error, prompt v3 both tiers) | L2, L3 | `labeled-v5_20260916-132745_3eac2d` | 120 (35 human) | 0.978 (all, n=120) | — | 72.5% | 60.8% | 0.992 verbatim / 0.995 support | $0.431 (CLI est.; 3 Opus calls) | 72.6 s |

## Notes

- **2026-09-16 · Plan 2A (prompt v3, structured record cites) — checkpoint MET (record cites/doc rose +11.7%, outside the ±10 % band on the upside — flagged, not a failure).** Strict citation fidelity 0.964 → 0.992 (llm-v3 → llm-v5; bar ≥ 0.98), non-verbatim cites 150 → 17 of 4335 emitted, unsupported 47 → 23; span IoU 0.975 → 0.978 (bar: within 0.01); record cites/doc 30.9 → 34.6 (bar: within 10 %). Prompt-only effect (llm-v2 → llm-v4): fidelity 0.962 → 0.992, IoU 0.912 → 0.953. Cost $0.331 → $0.431/doc, latency 37 → 73 s/doc (the object output is ~3× the tokens). Downstream target precision/recall vs gold 96.5%/79.0% → 96.5%/79.0%. Drift (prompt v3, 10 docs × 3 runs): 26/30 pairs at IoU ≥ 0.999 (prompt v2 was 30/30), record-cite set Jaccard 0.868, errors 0; unstable docs: g010, g012 — one run of each moved the span (g012: one run found no span at all), so prompt v3 is less run-to-run stable than v2 and a production deployment should pin cached results per document sha.
