...
from legallm.ocr_decision import OcrDecisionConfig, page_text_metrics, should_ocr_page
...
def check_ocr_ready(language: str = "eng", tessdata: str | None = None) -> tuple[bool, str, dict[str, Any]]:
    """
...
class OcrMeta:
    ...
    error: str | None = None
    # For analysis/debug
    per_page_native_metrics: list[dict[str, Any]] | None = None
    per_page_ocr_metrics: list[dict[str, Any]] | None = None
...
def _get_textpage_ocr(page: Any, *, language: str, dpi: int, full: bool, tessdata: str | None):
    """
...
def extract_text_pymupdf_selective_ocr(
    ...
    *,
    ...
    pages_text: list[str] = []
    native_metrics: list[dict[str, Any]] | None = [] if collect_debug_metrics else None
    ocr_metrics: list[dict[str, Any]] | None = [] if collect_debug_metrics else None
    ...
    try:
        ...
        for i in range(meta.total_pages):
            ...
            if collect_debug_metrics and native_metrics is not None:
                m = dict(details.get("metrics", {})) if details else dict(page_text_metrics(native))
            ...
            if do_ocr:
                ...
                if collect_debug_metrics and ocr_metrics is not None:
...
def _cli() -> int:
    ...
    ap.add_argument(
        "--cache-dir", type=str, default=None, help="Optional cache dir; if set, writes cache files and exits"
    )
