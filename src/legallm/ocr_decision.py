...
def should_ocr_document(
    ...
    *,
    ...
    header_lines = [
        ...
        for ln in sample
        if (
            _RE_PACER_HEADER.search(ln)
            or _RE_CA3_HEADER.search(ln)
            or _STAMP_LINE_RE.search(ln)
            or _PAGEID_RE.search(ln)
        )
    ]
    ...
    if (
        header_frac >= cfg.doc_header_line_frac
        and deheaded_len < cfg.doc_min_deheaded_len
        and deheaded_alpha < cfg.doc_min_deheaded_alpha
    ):
