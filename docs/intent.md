...
- [x] Public repo; README with architecture, **results table (baseline vs LLM v_final on gold: span-F1, field match, citation fidelity, judge score + κ, cost/doc, latency/doc)**, failure analysis, limitations
- [ ] Live demo URL works from a cold browser
- [x] CI green on `main`; a deliberately-regressed PR fails the gate (gate table in README; branch `demo/gate-regression`)
...
- 2026-09-15 · **Human review done (Noah, 35 docs) — judge validated:** rating κ 0.885 (93.9 % agreement, n=33), has-facts κ 1.00, span IoU human↔judge 0.935. Gold set complete: 120/120 (35 human, 85 judge-accepted, separable by `labeler`). Headline on human-only gold (n=35): llm-v2 IoU 0.869 (≥0.9 on 85.7%) vs rules_v2 0.647 (34.3%); on all gold (n=120): 0.912 vs 0.572; paired (all) wins 73 / ties 37 / losses 10. DoD items met: judge κ reported; results table has a human-labeled reference. Items 9 (judge) and 7 (results table) are therefore complete; remaining: 10 downstream, 11 routing, 12 API/UI, 13 CI gate, 14 README/publish.
- 2026-09-15 · **Item 10 done — downstream eval** (`downstream_eval.py` / `legallm-downstream`, `evals/runs/labeled-any_20260915-224235_c0ac4e/downstream.md`; resolver cache only, no network per R5; BM25 fitted on the canonical build's train split minus the 37 gold docs in it; targets fixed per doc = the gold span's resolved citations). **Resolution:** rules_v2 reaches ≥1 resolved citation on 42.5% of the 120 gold docs vs llm-v2 24.2% and the gold span itself 25.8% — but only 15.1% of the rules targets are citations the gold facts section actually contains (llm-v2 95.9%); rules emits 192 targets on 11 of the 33 no-facts docs (llm-v2: 0). So the pipeline's 58.5 % resolution figure was mostly Argument-section and TOC citations leaking into the span; a correct facts section cites few cases (record cites dominate). llm-v2 target recall vs gold 67.6% (misses = anchor failures + 10 uncached cites). **BM25 Recall@10** (n=31 gold docs with ≥1 target; build baseline on its own test split 0.104, n=131, not the 0.126/152 from the overwritten build): rules 0.126, llm-v1 0.145, llm-v2 0.120, gold 0.143; paired vs rules almost all ties — **no measurable retrieval effect at this n**, and the gold span itself does no better than rules, so span quality is not the bottleneck for the retrieval task (the BM25 baseline is). Headline finding for the README: *better spans give fewer but correct citation targets; the old resolution rate rewarded leakage.* Remaining: 11 routing, 12 API/UI, 13 CI gate, 14 README/publish.
- 2026-09-16 · **Item 11 done — routing / llm-v3** (`routing.py`, registered as `llm-v3`; `legallm-extract … --method llm-v3`). Cheap tier = llm-v2 (Sonnet 5); strong tier = Opus 5 at the same prompt v2, effort high (D2's escalation tier — the open question from v0.2.2 is now resolved: yes). Triggers: anchor_not_found (has_facts but unlocatable anchors), span validator flags, cheap-call error; never on a no-facts verdict; `unsupported_citation` available but off. Both passes are kept in provenance with summed cost/latency; the harness reuses the cached cheap result so a routed run costs only the escalations. Run `labeled-v3_20260916-001818_90b33c` (120 gold docs): 8 escalations, 8/8 spans recovered (7 at IoU ≥ 0.99); IoU 0.912 → 0.975, ≥0.9 share 90.0% → 95.8%; paired vs rules 77/40/3; cost $0.274 → $0.331/doc; downstream target recall 67.6% → 79.0% at 96.5% precision. `evals/RESULTS.md` re-rendered with four methods + downstream. Remaining: 12 API/UI/logging, 13 CI gate + deploy + injection guard, 14 README/ARCHITECTURE + publish.
- 2026-09-16 · **Item 12 done — C10 service, C11 UI, C12 observability.** `api.py` / `legallm-api`: FastAPI `POST /extract` (multipart PDF/.txt or JSON text; `method`, `doc_type_id`, `compare`) → `{doc, result: {extraction, validation}, baseline}`; `GET /health` reports methods, default method (`llm-v3`), backend and whether an LLM backend is usable (503 for llm-* otherwise, rules_v2 always works); 413 upload limit, 429 in-memory per-client rate limit, 422/400/502 mapped errors. Per-request JSONL log (`logs/requests.jsonl`, gitignored): method, model, prompt sha, tokens, cost, latency, validator flags, routing tier/trigger, doc sha — never document text. The rules baseline is computed on the identical `doc_text` (a re-normalisation bug caught by the live smoke shifted its offsets by 4 chars; fixed + regression test). `ui_app.py` / `legallm-ui`: Streamlit upload/paste → side-by-side rules_v2 vs chosen method (span preview, confidence, notes, validator flags, fields, provenance incl. routing tier and cost), in-process or against the API. `observability.py` / `legallm-dashboard` → `evals/dashboard.md`: requests per method/day (errors, span found, validator pass, cost total/per doc, latency p50/p95, escalation rate) + quality-over-time table from every `evals/runs/*/summary.json`. Live smoke on g001 via the service: rules_v2 0.7 s; llm-v3 27 s, Sonnet only, span [19461, 25754] (gold [19460, 25754]), CLI cost estimate $0.64 (subscription-billed). 17 offline tests (448 total). Scheduled drift check = CI smoke eval (item 13). Remaining: 13 CI gate + deploy + injection guard, 14 README/ARCHITECTURE + publish.
- 2026-09-16 · **Item 13 done — C13 gate, C14 deploy package, C15 guard.** *Gate:* `ci_gate.py` / `legallm-gate` runs the smoke eval (3 labeled synthetic briefs) **offline** — the Sonnet/Opus responses for them were recorded once via `claude-cli` and committed under `tests/fixtures/xeval_smoke/cache/` (llm-v2, llm-v3; rules runs live) — and compares per-method IoU / citation fidelity / support / span-found against `evals/ci/smoke_baseline.json` with D8 thresholds (−0.02 IoU, −0.01 fidelity, no span-found drop, zero errors); exit 1 fails the build, table goes to the job summary + artifact. CI workflow gains the `gate` job (after lint+test), a weekly Monday cron (the C12 drift check) and `workflow_dispatch` with a methods input. `--update-baseline` is the explicit way to accept an intended change. *Deploy:* `Dockerfile` (API, uvicorn, `$PORT`, env-only secrets) and `deploy/hf_space/{README.md,app.py,requirements.txt}` for the HF Streamlit Space (installs the repo from GitHub), with `deploy/README.md` steps; UI gains a per-session cap (`LEGALLM_UI_MAX_PER_SESSION`). **Not yet live:** pushing the Space and adding the `ANTHROPIC_API_KEY` secret need Noah's HF account and a funded key (the subscription backend cannot be hosted) — R3-style, his action. *Guard:* `guard.py`: 10 named injection patterns + zero-width/bidi-character detection on the document → `injection_suspected:<pattern>` validator flags (extraction still runs; `LEGALLM_GUARD_MODE=block` → 422 in the API), and an output-field scan (parties, posture, key events, notes, citations) that fails validation with `injection_in_output:<field>:<pattern>` — the model following the document instead of the prompt is a hard failure. Structural isolation unchanged: fixed system prompt per version, document only in the user turn, schema-constrained JSON, anchors re-located in the text, citations verified verbatim. Harness change: `--offline` now lets only rules_v2 run uncached. 23 new tests (472 total). Remaining: 14 README/ARCHITECTURE + publish (and Noah: HF Space + key).
- 2026-09-16 · **Item 14 — C16 docs done; C17 publish drafted.** README retitled *Legal Facts Extraction Service*: results table (rules_v2 / llm-v2 / llm-v3 on the 120-doc gold set + the human-only column), the headline finding (rules' resolution rate was TOC/Argument leakage — 15 % target precision vs 96 % for the LLM), the retrieval limitation stated plainly (BM25 flat across span sources incl. gold; April M4 result stands), failure analysis, label provenance with κ, architecture diagram, run/eval commands, the CI gate with a real FAIL table from branch `demo/gate-regression` (every located span truncated 15 %), security model, limitations incl. the two deliberate gaps (no agents; drift check nominal). `ARCHITECTURE.md`: workflow-not-agent, anchors-not-offsets, whole-doc-not-chunks (6/120 gold docs hit the 400k head window; facts sections stayed inside it), routing rule + effect, validators as the boundary, eval design, security model, cost/latency/scaling (≈ $330 per 1k docs with routing), next steps. `docs/publish.md`: LinkedIn draft, 60-s demo script, publish checklist. **Open, Noah's:** HF Space push + `ANTHROPIC_API_KEY` (then paste the URL into README), the post, the demo cut. **Gate correction (found while producing the README's FAIL table):** the item-13 gate replayed *finished extractions* from a cache, so a deliberate 15 % span truncation in `locate_span` passed it. Rebuilt: `replay.py` records raw model responses keyed by model+prompt+document (`tests/fixtures/xeval_smoke/responses.jsonl`, `legallm-gate --record`) and the gate runs the real extractors on a replay client, so anchor location, parsing, validators and routing execute in CI; the same regression now fails (llm IoU 0.955 → 0.867). Lesson logged in `ARCHITECTURE.md` §6: a gate is shown failing before it is trusted.
