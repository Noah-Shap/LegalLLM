...
from ocr_decision import OcrDecisionConfig, should_ocr_document

# Optional: OCR backend (PyMuPDF + Tesseract). If unavailable, pipeline can still run without OCR.
try:
    # Prefer the updated backend if present.
    from pymupdf_ocr_backend_updated import check_ocr_ready, extract_text_with_ocr_fallback
except Exception:  # pragma: no cover
    try:
        from pymupdf_ocr_backend import check_ocr_ready, extract_text_with_ocr_fallback
    except Exception:  # pragma: no cover
        check_ocr_ready = None
        extract_text_with_ocr_fallback = None
...
HEADING_LINE = re.compile(
    r"(?m)^\s*(?:[IVXLC]+\.)?\s*(?:\d+\.)?\s*([A-Z][A-Z0-9 \-–—/&,:]{2,90})\s*$"
)

BAD_HEADING_FRAGMENTS = [
    "TABLE OF CONTENTS", "TABLE OF AUTHORITIES", "JURISDICTION",
    "CERTIFICATE", "SERVICE", "COMPLIANCE", "CORPORATE DISCLOSURE",
    "CASE ", "DOCUMENT ", "FILED ", "PAGE "
]
...
def extract_facts_span(clean: str) -> Tuple[Optional[Tuple[int, int]], Dict[str, Any]]:
...
RE_ARG_MARKERS = re.compile(r"\b(we\s+argue|this\s+court\s+should|standard\s+of\s+review|for\s+these\s+reasons)\b", re.I)
...
# -----------------------------
def search_recap_documents(s: requests.Session, query: str, max_docs: int, search_fields: Optional[str]) -> List[Dict[str, Any]]:
    url = f"{BASE}/search/"
    params = {
        "q": query,
...
PDF_URL_KEYS = [
    "download_url", "file_url", "pdf_url", "download", "url"
]
WEB_URL_KEYS = [
    "absolute_url", "permalink", "web_url"
]
...
def extract_pdf_url_from_webpage(s: requests.Session, web_url: str) -> Optional[str]:
    """
...
def derive_pdf_url(s: requests.Session, item: Dict[str, Any]) -> Tuple[Optional[str], Dict[str, Any]]:
    meta = {"strategy": None, "web_url": None}
...
def extract_text_pypdf(pdf_path: Path, warnings_log_path: Optional[Path] = None) -> Optional[str]:
    try:
...
def looks_like_garbage_text(text: str) -> bool:
    ...
    """
    from ocr_decision import looks_like_garbage_text as _looks_like_garbage
...
def extract_text_fitz(pdf_path: Path, warnings_log_path: Path) -> Optional[str]:
...
def get_pdf_page_count_fast(pdf_path: Path) -> Optional[int]:
...
def extract_text_best(pdf_path: Path, warnings_log_path: Path) -> tuple[Optional[str], str, list[str]]:
...
def main():
    ...
    ap.add_argument("--search_fields", type=str, default="id,absolute_url,download_url,short_description,document_type,description",
                    help="Attempt to request these fields from Search API (if supported).")
    ap.add_argument("--no_scope_filter", action="store_true", help="Disable brief-scope filtering (include all PACER documents returned by search).")
    ap.add_argument("--exclude_motion_briefs", action="store_true", help="Exclude briefs that are actually motion/memo filings (based on description).")

    # OCR integration (optional)
    ap.add_argument("--enable_ocr", action="store_true",
                    help="If set, attempt PyMuPDF+Tesseract OCR for PDFs flagged needs_ocr.")
    ...
    ap.add_argument("--tessdata", type=str, default=None,
                    help="Optional tessdata directory (overrides TESSDATA_PREFIX).")
    ap.add_argument("--ocr_timeout_seconds", type=int, default=900,
                    help="Safety limit on OCR runtime per document (seconds).")
    ap.add_argument("--ocr_max_pages", type=int, default=300,
                    help="Safety limit: abort OCR if PDF exceeds this many pages.")
    ap.add_argument("--ocr_save_txt", action="store_true",
                    help="If set, write OCR output to a .txt file next to the PDF.")
    ap.add_argument("--force_ocr", action="store_true",
                    help="Force OCR on every page when OCR is invoked (slow).")
    args = ap.parse_args()
    ...
    def _maybe_run_ocr(pdf_path: Path) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
    ...
    with open(args.failures_jsonl, "w", encoding="utf-8") as f_fail:
        for item in tqdm(hits, desc="Processing", file=sys.stdout):
            ...
            if (not args.no_scope_filter) and (not in_scope_brief(item, exclude_motion_briefs=args.exclude_motion_briefs)):
                failures += 1
                f_fail.write(json.dumps({
                    "reason": "out_of_scope",
                    "search_id": search_id,
